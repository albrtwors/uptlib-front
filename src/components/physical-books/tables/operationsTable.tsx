        ...
        <div className="p-3 bg-white rounded-xl shadow-sm border border-gray-200">
                        ...
                        {operations.map((operation: any) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-4 font-medium text-gray-900 max-w-[200px] truncate md:max-w-none">
                                    ...
                                    <div className="text-sm text-gray-500">ID Libro: {operation.book?.id}</div>
                                </td>
            ...
            <div className="md:hidden space-y-3">
                {operations.map((operation: any) => (
                    <div key={operation.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all">
                        {/* Encabezado: Título del Libro y Tipo de Operación */}
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex-1 pr-2">
                                <h3 className="font-bold text-base text-gray-900 line-clamp-2">
                                    {operation.book?.title}
                                </h3>
                                <p className="text-xs text-gray-400 mt-0.5">ID Libro: {operation.book?.id}</p>
                            </div>

                            {/* Badge dinámico de color según tipo de operación */}
                            <span className={`px-2.5 py-1 text-xs font-bold rounded-full shrink-0 ${operation.type === 'PRESTAMO' ? 'bg-yellow-100 text-yellow-800' :
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm text-gray-600 mb-4">
                            ...
                            <div className="col-span-2">
                                ...
                                <span className="text-gray-900 text-xs block mt-0.5 font-medium">
                                    {new Date(operation.createdAt).toLocaleString('es-ES')}
                                </span>
                            </div>
                        </div>
            ...
            {operations.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-sm font-medium">No hay operaciones de libros registradas</p>
                </div>
            )}
        </div>
    );
