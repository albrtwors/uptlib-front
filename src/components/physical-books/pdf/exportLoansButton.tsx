...
import LoansPDF from './LoansPDF';
import BookLoansPDF from './LoansPDF';

export default function ExportLoansPDFButton({ items, title }: any) {
    const [instance, updateInstance] = usePDF({
        document: <BookLoansPDF items={items} title={title || 'Reporte de Items'} />,
    });

    useEffect(() => {
        updateInstance(<BookLoansPDF items={items} title={title} />);
