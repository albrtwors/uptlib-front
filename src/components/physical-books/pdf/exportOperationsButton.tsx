...
import BookOperationsPDF from './operationsPDF';

export default function ExportPDFButton({ items, title }: any) {
    const [instance, updateInstance] = usePDF({
        document: <BookOperationsPDF items={items} title={title || 'Reporte de Items'} />,
    });

    useEffect(() => {
        updateInstance(<BookOperationsPDF items={items} title={title} />);
